...
async def create_optimization_job(
    ...
    try:
        ...
        # Check for pending/running jobs for this scenario
        ...
        if active_jobs and len(active_jobs) >= 3:
            raise HTTPException(
                status_code=429,
                detail={
                    "error_type": "TOO_MANY_ACTIVE_JOBS",
                    "message": f"Zbyt wiele aktywnych zadań dla tego scenariusza ({len(active_jobs)}/3)",
                    "details": "Maksymalnie 3 zadania mogą być aktywne jednocześnie dla jednego scenariusza",
